...
def listar(usuario_cc: str, db: Session = Depends(get_db)):
...
def obtener(id: int, usuario_cc: str, db: Session = Depends(get_db)):
...
def principal(usuario_cc: str, db: Session = Depends(get_db)):
    stmt = select(models.MetodoPago).where(
        models.MetodoPago.k_usuario_cc == usuario_cc,
...
async def actualizar(id: int, usuario_cc: str, data: schemas.MetodoPagoUpdate, db: Session = Depends(get_db)):
    mp = db.get(models.MetodoPago, id)
    if not mp or mp.k_usuario_cc != usuario_cc:
        raise HTTPException(status_code=404, detail="Método de pago no encontrado")
...
async def eliminar(id: int, usuario_cc: str, db: Session = Depends(get_db)):
    mp = db.get(models.MetodoPago, id)
    if not mp or mp.k_usuario_cc != usuario_cc:
        raise HTTPException(status_code=404, detail="Método de pago no encontrado")
...
async def set_principal(id: int, usuario_cc: str, db: Session = Depends(get_db)):
    mp = db.get(models.MetodoPago, id)
    if not mp or mp.k_usuario_cc != usuario_cc:
        raise HTTPException(status_code=404, detail="Método de pago no encontrado")
    if not mp.b_activo:
...
def consultar_saldo_total(usuario_cc: str, db: Session = Depends(get_db)):
    """
