...
class MetodoPagoBase(BaseModel):
    k_usuario_cc: int = Field(..., gt=0)  # Changed from str to int to match database type
...
class MetodoPagoOut(BaseModel):
    k_metodo_pago: int
    k_usuario_cc: int  # Changed from str to int to match database type
    t_tipo_tarjeta: str
...
class RecargaSaldoResponse(BaseModel):
    ...
    k_metodo_pago: int
    k_usuario_cc: int  # Changed from str to int
    monto_recargado: int
    saldo_anterior: int
    saldo_nuevo: int
    n_marca: Optional[str]
