...
COMMENT ON COLUMN usuario.hashed_password IS 'Password del usuario almacenada de forma hasheada (bcrypt u otro algoritmo).';
COMMENT ON COLUMN usuario.is_verified IS 'Indica si el usuario verificó su cuenta/ correo. false por defecto.';
COMMENT ON COLUMN usuario.otp IS 'Codigo OTP temporal usado para verificacion. Puede ser NULL.';
COMMENT ON COLUMN usuario.date_created IS 'Fecha y hora de creacion del registro (timestamp).';
