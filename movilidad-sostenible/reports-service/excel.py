...
def _safe_cell_value(value: Any) -> Any:
    """Convert complex values to a string so Excel serialization does not fail."""
...
def build_admin_overview_excel(overview: Dict[str, Any]) -> bytes:
    ...
    ws.append(["Travels", overview.get("travels_count", 0)])
    ws.append(["Fines", overview.get("fines_count", 0)])
    ws.append(["Complaints", overview.get("complaints_count", 0)])
    ws.append(["Maintenances", overview.get("maintenance_count", 0)])
    ws.append(["Complaints by status", str(overview.get("complaints_by_status", {}))])
    ws.append(["Complaints by type", str(overview.get("complaints_by_type", {}))])
    ws.append(["Maintenance by entity", str(overview.get("maintenance_by_entity", {}))])
    ws.append([])

    _write_table(ws, "Users", overview.get("users", []))
    _write_table(ws, "Bicycles", overview.get("bicycles", []))
    _write_table(ws, "Travels", overview.get("travels", []))
    _write_table(ws, "Fines", overview.get("fines", []))
    _write_table(ws, "Complaints", overview.get("complaints", []))
    _write_table(ws, "Maintenances", overview.get("maintenances", []))
...
def build_user_dashboard_excel(dashboard: Dict[str, Any]) -> bytes:
    ...
    ws.append(["Travels", dashboard.get("travels_count", 0)])
    ws.append(["Complaints", dashboard.get("complaints_count", 0)])
    ...
    _write_table(ws, "Travels", dashboard.get("travels", []))
    _write_table(ws, "Complaints", dashboard.get("complaints", []))
