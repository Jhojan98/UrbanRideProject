...
/*  Created On : 24-nov.-2025 11:28:23 p. m. 				*/
/*  DBMS       : PostgreSQL 						*/
/* ---------------------------------------------------- */
...
(
	...
	t_padlock_status varchar(50) NOT NULL   DEFAULT 'LOCK',	-- Estado del candado de la bicicleta.
...
(
	...
	d_descripcion varchar(250) NOT NULL	-- Descripcion de la multa.
)
;
...
ALTER TABLE bicycle ADD CONSTRAINT "CHK_t_padlock_status" CHECK (t_padlock_status IN ('LOCK', 'OPEN'))
;
...
COMMENT ON COLUMN fine.d_descripcion
...
(
	k_id_maintenance serial NOT NULL,
	k_id_bicycle integer NOT NULL,
	t_maintenance_type varchar(50) NOT NULL, -- PREVENTIVO | CORRECTIVO | EMERGENCIA
	f_scheduled_date timestamp without time zone NOT NULL,
	f_completed_date timestamp without time zone NULL,
	v_total_trips integer NULL,
	t_triggered_by varchar(50) NOT NULL, -- KILOMETRAJE | TIEMPO | ALERTA_IOT | USUARIO
	k_technician_id varchar(50) NOT NULL, -- SIN ESTO
	k_id_station integer NOT NULL, -- Y SIN ESTO xd 
	v_cost_usd numeric(10,2) NULL,
	n_notes varchar(500) NULL
);

/* Create Primary Keys */
ALTER TABLE maintenance_record ADD CONSTRAINT "PK_maintenance_record"
	PRIMARY KEY (k_id_maintenance);

/* Create Foreign Keys */
ALTER TABLE maintenance_record ADD CONSTRAINT "FK_maintenance_bicycle"
	FOREIGN KEY (k_id_bicycle) REFERENCES bicycle (k_id_bicycle) ON DELETE No Action ON UPDATE No Action;

ALTER TABLE maintenance_record ADD CONSTRAINT "FK_maintenance_station"
	FOREIGN KEY (k_id_station) REFERENCES station (k_id_station) ON DELETE No Action ON UPDATE No Action;

/* Create Indexes */
CREATE INDEX "IXFK_maintenance_bicycle" ON maintenance_record (k_id_bicycle ASC);
CREATE INDEX "IXFK_maintenance_station" ON maintenance_record (k_id_station ASC);

/* Comments */
COMMENT ON TABLE maintenance_record IS 'Tabla para registrar el mantenimiento de las bicicletas.';
COMMENT ON COLUMN maintenance_record.k_id_maintenance IS 'Llave primaria del registro de mantenimiento.';
COMMENT ON COLUMN maintenance_record.k_id_bicycle IS 'ID de la bicicleta mantenida.';
COMMENT ON COLUMN maintenance_record.t_maintenance_type IS 'Tipo de mantenimiento: PREVENTIVO, CORRECTIVO, EMERGENCIA.';
COMMENT ON COLUMN maintenance_record.f_scheduled_date IS 'Fecha programada del mantenimiento.';
COMMENT ON COLUMN maintenance_record.f_completed_date IS 'Fecha de finalización del mantenimiento.';
COMMENT ON COLUMN maintenance_record.v_total_trips IS 'Total de viajes de la bicicleta al momento del mantenimiento.';
COMMENT ON COLUMN maintenance_record.t_triggered_by IS 'Origen del mantenimiento: KILOMETRAJE, TIEMPO, ALERTA_IOT, USUARIO.';
COMMENT ON COLUMN maintenance_record.k_technician_id IS 'ID del técnico responsable.';
COMMENT ON COLUMN maintenance_record.k_id_station IS 'Estación donde se realiza el mantenimiento.';
COMMENT ON COLUMN maintenance_record.v_cost_usd IS 'Costo del mantenimiento en USD.';
COMMENT ON COLUMN maintenance_record.n_notes IS 'Notas adicionales del mantenimiento.';

/* Roles */
CREATE ROLE manager_maintenance LOGIN PASSWORD 'g_maintenance';
GRANT SELECT, INSERT, UPDATE, DELETE ON public.maintenance_record TO manager_maintenance;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO manager_maintenance;