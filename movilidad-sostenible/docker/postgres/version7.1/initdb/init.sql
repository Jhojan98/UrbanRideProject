...
(
	...
	t_entity_tipe varchar(50) NOT NULL, -- Tipo de entidad que realiza el mantenimiento, BICYCLE, STATION, LOCK.
	t_maintenace_type varchar(50) NOT NULL, -- Tipo de mantenimiento realizado, PREVENTIVE, CORRECTIVE, INSPECTION.
	t_triggered_by varchar(50) NOT NULL, -- Indica quién o qué generó la necesidad del mantenimiento, ya sea un usuario, un sistema automatizado o un técnico.
	...
	v_cost integer NULL, -- Costo asociado al mantenimiento realizado, permitiendo llevar un control financiero de las intervenciones efectuadas.

	k_id_bicycle varchar(11) NULL,	-- Identificador único de la bicicleta a la que se le realizó el mantenimiento.
	k_id_station integer NULL,	-- Identificador único de la estación a la que se le realizó el mantenimiento.
	k_id_lock varchar(50) NULL	-- Identificador único del candado a la que se le realizó el mantenimiento.
...
ALTER TABLE maintenance ADD CONSTRAINT "CHK_t_entity_tipe"
	CHECK (t_entity_tipe IN ('BICYCLE', 'STATION', 'LOCK'))
;

ALTER TABLE maintenance ADD CONSTRAINT "CHK_t_maintenace_type"
	CHECK (t_maintenace_type IN ('PREVENTIVE', 'CORRECTIVE', 'INSPECTION'))
;

ALTER TABLE maintenance ADD CONSTRAINT "CHK_t_status" CHECK (t_status IN ('PENDING', 'SOLVING', 'RESOLVED'))
;

ALTER TABLE maintenance ADD CONSTRAINT "CHK_t_triggered_by"
	CHECK (t_triggered_by IN ('ADMIN', 'USER', 'IOT_ALERT'))
;

CREATE INDEX "IXFK_maintenance_bicycle" ON maintenance (k_id_bicycle ASC)
;

CREATE INDEX "IXFK_maintenance_station" ON maintenance (k_id_station ASC)
;

CREATE INDEX "IXFK_maintenance_lock" ON maintenance (k_id_lock ASC)
;
...
ALTER TABLE maintenance ADD CONSTRAINT "FK_maintenance_lock"
	FOREIGN KEY (k_id_lock) REFERENCES slots (k_id_slot) ON DELETE No Action ON UPDATE No Action
;
